...
- `project/domains/<domain>/research.md`
- 当前新增材料
...
- `research.md`：交付参考工作台，用于记录来源、前期规划、会议增量、交付参考和待确认问题
- `project/overview.md`：用于同步领域清单、当前阶段、G 状态和项目级共享信息

## 文档边界

- `design.md` 只承接会影响 AI / demo 实现的稳定设计结论，不直接堆原始材料。
- `design.md` 默认继承 `project/ui-brand.md`；只有领域特有的设计例外才写进 `design.md`。
- `research.md` 主要服务后续交付和测试整理，不要求逐项回写 `design.md`。
- `tech/` 中的人写技术设计默认服务后续交付参考，不要求同步进 `design.md`；只有它会改变 demo 表现时才需要回写。
- 如果页面结构、交互或 Mock 方案变化，直接更新 `design.md`。
- 如果新增结论属于项目级默认设计规范，应更新 `project/ui-brand.md`，而不是分散写进多个领域文档。
- `research.md` 不替代 `project/domains/<domain>/delivery/` 下的正式交付文档。
...
5. 需要对外交付时，再把 `research.md` 中的参考点提炼到 `delivery/*`

- 如果材料已经明确只影响 demo 设计，可以直接由使用者更新 `design.md`；除非还需要保留交付或测试参考，否则 `research.md` 无需重复登记。
- 如果材料属于项目级默认视觉或品牌规范，优先补到 `project/ui-brand.md`。
- 如果材料主要影响交付规则、测试依据或评审口径，留在 `research.md` 即可，不阻塞 `design` 或 `build`。
- 如果材料同时影响 demo 设计和交付表达，可以直接更新 `design.md`，并在 `research.md` 中保留简短摘要供后续交付使用。
...
- 需要留痕的交付 / 测试参考材料已按需整理到 `research.md`
